...
public class SavingBlockchainService {
    @Value("${saving.contractAddress}")
    private String contractAddress;


    private SavingContract loadContract(String privateKey) {
        //web3j 인스턴스 생성
        ...
        return SavingContract.load(contractAddress, web3j, transactionManager, dynamicGasProvider);
    }
    ...
    public String openAccount(int savingOptionId, BigInteger amount, BigDecimal interestRate, BigInteger maturityPeriodInSeconds, String privateKey) throws Exception {
        SavingContract savingContract = loadContract(privateKey);
        //정수값으로 변형
        BigInteger scaledInterestRate = interestRate.multiply(new BigDecimal("100")).setScale(0, RoundingMode.DOWN).toBigInteger();

        return savingContract.openAccount(BigInteger.valueOf(savingOptionId), amount, scaledInterestRate, maturityPeriodInSeconds).send().getTransactionHash();
    }

    //적금넣기 메서드
    public String depositSavingAccount(int optionId,BigDecimal amount,String privateKey) throws Exception {
        SavingContract savingContract = loadContract(privateKey);
        return savingContract.deposit(BigInteger.valueOf(optionId), amount.toBigInteger()).send().getTransactionHash();
    }
    ...
    public String returnPrincipal(int savingOptionId,String privateKey) throws Exception {
        SavingContract savingContract = loadContract(privateKey);
        return savingContract.returnPrincipal(BigInteger.valueOf(savingOptionId)).send().getTransactionHash();
    }
